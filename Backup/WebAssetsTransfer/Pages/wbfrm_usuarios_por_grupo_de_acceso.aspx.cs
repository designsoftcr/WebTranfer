...
namespace Modulo_Boston.Pages
{
    public partial class wbfrm_usuarios_por_grupo_de_acceso : Page
    {
            ...
            if (CheckPopulateControls() && !string.IsNullOrEmpty(status_control.Value))
            {
                if (status_control.Value == "Edit" &&  !string.IsNullOrEmpty(this.empleado_code_old.Value))
                {
                    ...
                    if (rb_estado.SelectedValue == "activado")
                     ...
                     bool transaction = new cls_usuarios_por_grupo_de_acceso().update_cusuarios_por_grupo_de_acceso(this.empleado_code_old.Value, this.tb_empeado_code.Text, Convert.ToInt32(this.tb_code_grupo.Text), this.tb_compania.Text, this.tb_id_usuario.Text, estado);
                     if (transaction)
                ...
                else if (status_control.Value == "Create")
                {
                    ...
                    bool transaction = new cls_usuarios_por_grupo_de_acceso().create_cusuarios_por_grupo_de_acceso(this.tb_empeado_code.Text, Convert.ToInt32(this.tb_code_grupo.Text), this.tb_compania.Text, this.tb_id_usuario.Text, estado);
                    if (transaction)
                        this.crear_mensajes("success", "Employee has succesufully added!");
                    else
                        this.crear_mensajes("error", "Employee are not added!");
                    ClearControls();
                    ClearControls();
                }
        ...
        protected void img_btn_delete_Click(object sender, EventArgs e)
        {
            ...
            {
                bool transaction = new cls_usuarios_por_grupo_de_acceso().delete_cusuarios_por_grupo_de_acceso(this.empleado_code_old.Value);
                if (transaction)
                    this.crear_mensajes("success", "Employee was succesufully deleted!");
                else
                    this.crear_mensajes("error", "Employee was not deleted!!");
